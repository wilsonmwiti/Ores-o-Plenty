...
public abstract class MinecraftServer extends RecursiveEventLoop<TickDelayedTask> implements ISnooperInfo, ICommandSource, AutoCloseable, Runnable {
   ...
   protected void updateTimeLightAndEntities(BooleanSupplier hasTimeLeft) {
      ...
      for(ServerWorld serverworld : this.getWorlds()) {
         ...
         tickTimes.computeIfAbsent(serverworld.getDimension().getType(), k -> new long[100])[this.tickCounter % 100] = Util.nanoTime() - tickStart;
      }
   ...
   private void loadDataPacks(WorldInfo worldInfoIn) {
      ...
      CompletableFuture<Unit> completablefuture = this.resourceManager.reloadResourcesAndThen(this.backgroundExecutor, this, list1, CompletableFuture.completedFuture(Unit.INSTANCE));
