...
public class ChunkStatus extends net.minecraftforge.registries.ForgeRegistryEntry<ChunkStatus> {
   ...
   public static final ChunkStatus EMPTY = registerSelective("empty", (ChunkStatus)null, -1, field_222618_n, ChunkStatus.Type.PROTOCHUNK, (p_223194_0_, p_223194_1_, p_223194_2_, p_223194_3_) -> {
   });
   public static final ChunkStatus STRUCTURE_STARTS = register("structure_starts", EMPTY, 0, field_222618_n, ChunkStatus.Type.PROTOCHUNK, (p_222586_0_, p_222586_1_, p_222586_2_, p_222586_3_, p_222586_4_, p_222586_5_, p_222586_6_, p_222586_7_) -> {
   ...
   public static final ChunkStatus STRUCTURE_REFERENCES = registerSelective("structure_references", STRUCTURE_STARTS, 8, field_222618_n, ChunkStatus.Type.PROTOCHUNK, (p_222587_0_, p_222587_1_, p_222587_2_, p_222587_3_) -> {
      p_222587_1_.generateStructureStarts(new WorldGenRegion(p_222587_0_, p_222587_2_), p_222587_3_);
   });
   public static final ChunkStatus BIOMES = registerSelective("biomes", STRUCTURE_REFERENCES, 0, field_222618_n, ChunkStatus.Type.PROTOCHUNK, (p_222594_0_, p_222594_1_, p_222594_2_, p_222594_3_) -> {
      p_222594_1_.generateBiomes(p_222594_3_);
   });
   public static final ChunkStatus NOISE = registerSelective("noise", BIOMES, 8, field_222618_n, ChunkStatus.Type.PROTOCHUNK, (p_222592_0_, p_222592_1_, p_222592_2_, p_222592_3_) -> {
      p_222592_1_.makeBase(new WorldGenRegion(p_222592_0_, p_222592_2_), p_222592_3_);
   });
   public static final ChunkStatus SURFACE = registerSelective("surface", NOISE, 0, field_222618_n, ChunkStatus.Type.PROTOCHUNK, (p_222589_0_, p_222589_1_, p_222589_2_, p_222589_3_) -> {
      p_222589_1_.generateSurface(p_222589_3_);
   });
   public static final ChunkStatus CARVERS = registerSelective("carvers", SURFACE, 0, field_222618_n, ChunkStatus.Type.PROTOCHUNK, (p_222590_0_, p_222590_1_, p_222590_2_, p_222590_3_) -> {
      p_222590_1_.carve(p_222590_3_, GenerationStage.Carving.AIR);
   });
   public static final ChunkStatus LIQUID_CARVERS = registerSelective("liquid_carvers", CARVERS, 0, field_222619_o, ChunkStatus.Type.PROTOCHUNK, (p_222601_0_, p_222601_1_, p_222601_2_, p_222601_3_) -> {
      p_222601_1_.carve(p_222601_3_, GenerationStage.Carving.LIQUID);
   });
   public static final ChunkStatus FEATURES = register("features", LIQUID_CARVERS, 8, field_222619_o, ChunkStatus.Type.PROTOCHUNK, (p_222605_0_, p_222605_1_, p_222605_2_, p_222605_3_, p_222605_4_, p_222605_5_, p_222605_6_, p_222605_7_) -> {
   ...
   public static final ChunkStatus SPAWN = registerSelective("spawn", LIGHT, 0, field_222619_o, ChunkStatus.Type.PROTOCHUNK, (p_222602_0_, p_222602_1_, p_222602_2_, p_222602_3_) -> {
      p_222602_1_.spawnMobs(new WorldGenRegion(p_222602_0_, p_222602_2_));
   });
   public static final ChunkStatus HEIGHTMAPS = registerSelective("heightmaps", SPAWN, 0, field_222619_o, ChunkStatus.Type.PROTOCHUNK, (p_222603_0_, p_222603_1_, p_222603_2_, p_222603_3_) -> {
   });
   ...
   private static final IntList field_222621_q = Util.make(new IntArrayList(getAll().size()), (p_223202_0_) -> {
      ...
      for(int j = getAll().size() - 1; j >= 0; --j) {
         while(i + 1 < field_222620_p.size() && j <= field_222620_p.get(i + 1).ordinal()) {
            ++i;
         }
   ...
   private final ChunkStatus.ILoadingWorker field_223627_w;
   ...
   public ChunkStatus(String nameIn, @Nullable ChunkStatus parentIn, int taskRangeIn, EnumSet<Heightmap.Type> heightmapsIn, ChunkStatus.Type typeIn, ChunkStatus.IGenerationWorker generationWorkerIn, ChunkStatus.ILoadingWorker loadingWorkerIn) {
      ...
      this.field_223627_w = loadingWorkerIn;
   ...
   public CompletableFuture<Either<IChunk, ChunkHolder.IChunkLoadingError>> func_223201_a(ServerWorld worldIn, TemplateManager templateManagerIn, ServerWorldLightManager lightManager, Function<IChunk, CompletableFuture<Either<IChunk, ChunkHolder.IChunkLoadingError>>> p_223201_4_, IChunk p_223201_5_) {
      return this.field_223627_w.doWork(this, worldIn, templateManagerIn, lightManager, p_223201_4_, p_223201_5_);
   }
