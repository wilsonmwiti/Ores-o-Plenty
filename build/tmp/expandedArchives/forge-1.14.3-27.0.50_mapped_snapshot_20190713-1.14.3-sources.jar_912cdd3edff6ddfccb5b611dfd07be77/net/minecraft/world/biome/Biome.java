...
public abstract class Biome extends net.minecraftforge.registries.ForgeRegistryEntry<Biome> {
   ...
   public static final PerlinNoiseGenerator INFO_NOISE = new PerlinNoiseGenerator(new Random(2345L), 1);
   @Nullable
   ...
   public static <F extends IFeatureConfig, D extends IPlacementConfig> ConfiguredFeature<?> createDecoratedFeature(Feature<F> featureIn, F config, Placement<D> placementIn, D placementConfig) {
      Feature<DecoratedFeatureConfig> feature = featureIn instanceof FlowersFeature ? Feature.DECORATED_FLOWER : Feature.DECORATED;
      return new ConfiguredFeature<>(feature, new DecoratedFeatureConfig(featureIn, config, placementIn, placementConfig));
   }
