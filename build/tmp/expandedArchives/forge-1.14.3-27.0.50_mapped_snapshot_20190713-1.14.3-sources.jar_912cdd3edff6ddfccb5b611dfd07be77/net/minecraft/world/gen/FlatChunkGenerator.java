...
public class FlatChunkGenerator extends ChunkGenerator<FlatGenerationSettings> {
   ...
   private Biome func_202099_e() {
         ...
         if (configuredfeature != null) {
            for(ConfiguredFeature<?> configuredfeature1 : configuredfeature) {
               ...
               if (configuredfeature2.feature instanceof Structure) {
                  ...
                  flatchunkgenerator$wrapperbiome.addStructure(structure, ifeatureconfig != null ? ifeatureconfig : FlatGenerationSettings.FEATURE_CONFIGS.get(configuredfeature1));
               }
            }
         }
      }
      ...
      for(int i = 0; i < ablockstate.length; ++i) {
         ...
         if (blockstate != null && !Heightmap.Type.MOTION_BLOCKING.func_222684_d().test(blockstate)) {
            ...
            flatchunkgenerator$wrapperbiome.addFeature(GenerationStage.Decoration.TOP_LAYER_MODIFICATION, Biome.createDecoratedFeature(Feature.FILL_LAYER, new FillLayerConfig(i, blockstate), Placement.NOPE, IPlacementConfig.NO_PLACEMENT_CONFIG));
         }
      }
   ...
   public void makeBase(IWorld p_222537_1_, IChunk p_222537_2_) {
      ...
      Heightmap heightmap = p_222537_2_.func_217303_b(Heightmap.Type.OCEAN_FLOOR_WG);
      Heightmap heightmap1 = p_222537_2_.func_217303_b(Heightmap.Type.WORLD_SURFACE_WG);
      ...
      for(int i = 0; i < ablockstate.length; ++i) {
         ...
         if (blockstate != null) {
            for(int j = 0; j < 16; ++j) {
               for(int k = 0; k < 16; ++k) {
                  p_222537_2_.setBlockState(blockpos$mutableblockpos.setPos(j, i, k), blockstate, false);
