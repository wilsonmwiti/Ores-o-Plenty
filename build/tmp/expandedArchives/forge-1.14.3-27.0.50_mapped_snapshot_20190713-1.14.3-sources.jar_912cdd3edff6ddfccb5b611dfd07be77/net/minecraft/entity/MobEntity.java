...
public abstract class MobEntity extends LivingEntity {
   ...
   protected final void updateEntityActionState() {
      ++this.idleTime;
      this.world.getProfiler().startSection("checkDespawn");
      this.checkDespawn();
      this.world.getProfiler().endSection();
   ...
   public void faceEntity(Entity entityIn, float maxYawIncrease, float maxPitchIncrease) {
      ...
      if (entityIn instanceof LivingEntity) {
         ...
         d1 = livingentity.posY + (double)livingentity.getEyeHeight() - (this.posY + (double)this.getEyeHeight());
      } else {
   ...
   protected void dropSpecialItems(DamageSource source, int looting, boolean recentlyHitIn) {
      ...
      for(EquipmentSlotType equipmentslottype : EquipmentSlotType.values()) {
         ...
         boolean flag = f > 1.0F;
         if (!itemstack.isEmpty() && !EnchantmentHelper.hasVanishingCurse(itemstack) && (recentlyHitIn || flag) && this.rand.nextFloat() - (float)looting * 0.01F < f) {
   ...
   protected boolean isInDaylight() {
      if (this.world.isDaytime() && !this.world.isRemote) {
         ...
         if (f > 0.5F && this.rand.nextFloat() * 30.0F < (f - 0.4F) * 2.0F && this.world.isSkyLightMax(blockpos)) {
            return true;
         }
      }
