...
public class GroundPathNavigator extends PathNavigator {
   ...
   protected void trimPath() {
      ...
      if (this.shouldAvoidSun) {
         if (this.world.isSkyLightMax(new BlockPos(this.entity.posX, this.entity.getBoundingBox().minY + 0.5D, this.entity.posZ))) {
            return;
         }

         for(int i = 0; i < this.currentPath.getCurrentPathLength(); ++i) {
            PathPoint pathpoint = this.currentPath.getPathPointFromIndex(i);
            if (this.world.isSkyLightMax(new BlockPos(pathpoint.x, pathpoint.y, pathpoint.z))) {
